...
> [!IMPORTANT]
> 這是 [kerryandjry/ainto-app](https://github.com/kerryandjry/ainto-app) 維護的個人 GPLv3 fork，保留原作者來源與授權。
> 上方下載按鈕仍指向原作者版本，不是這個 fork。此版本已移除 Snippets，保留既有資料但停用其 alias。
> 更新採手動安裝；沒有 downstream 簽署／公證發行版。版本政策及已知限制見 [downstream 說明](docs/downstream.md)。
...
- 剪貼簿資料都保存在 `~/.config/ainto/`。Ainto 暫時貼上產生的文字時，會保留原本的剪貼簿 representations。
...
- **Rust 核心。** 應用程式探索、模糊搜尋、frecency 排序、剪貼簿儲存與 AI 指令，全部位於一個連結進 app 的 Rust 靜態函式庫中。
...
- **輸入。** 非啟動式 `NSPanel` 會保留前景 app。Launcher 與選用的 target shortcuts 使用註冊的系統快速鍵。
- **本地優先。** 所有資料都存在 `~/.config/ainto/` 之下：剪貼簿歷史用 SQLite，設定、AI 指令與排序則用 TOML。無遙測。
- **更新。** 此個人 fork 停用原版 Sparkle feed，避免被原版覆蓋。目前只提供本機建置與手動更新。
